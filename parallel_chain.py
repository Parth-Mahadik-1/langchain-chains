...
llm1=HuggingFaceEndpoint(
        repo_id="deepseek-ai/DeepSeek-R1",  # ✅ chat-capable
        temperature=0.7,
        max_new_tokens=512,
    )
...
prompt1 = PromptTemplate(
    template="provide notes on the text in format of definition , 5 key point , use case , real world example , advantages and disadvanges --> {text}",
    input_types=
    {
        "text":str
    }
...
prompt2 = PromptTemplate(
    template="make 5 question answer quiz on text --> {text}",
    input_types=
    {
        "text":str
    }
...
prompt3 = PromptTemplate(
    template="merge the notes and quiz in one document notes--> {notes} and quiz -- > {quiz}",
    input_types=
    {
        "notes":str,
        "quiz":str
